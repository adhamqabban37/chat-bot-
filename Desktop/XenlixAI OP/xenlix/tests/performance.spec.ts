...
test.describe('Performance and Lighthouse Audits', () => {
  ...
  test('should have good Core Web Vitals', async ({ page }) => {
    ...
    for (const pageInfo of pagesToTest) {
      ...
      expect(navigationTiming.domContentLoadedEventEnd - navigationTiming.fetchStart).toBeLessThan(3000); // DOMContentLoaded < 3s
      
      console.log(`${pageInfo.name} - Load time: ${navigationTiming.loadEventEnd - navigationTiming.fetchStart}ms`);
    }
  });
