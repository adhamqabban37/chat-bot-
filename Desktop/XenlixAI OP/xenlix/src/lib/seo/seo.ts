...
export function buildSeoChecklist(biz: NormalizedBusiness): SeoChecklistItem[] {
  ...
  checklist.push({
    ...
    description: 'Generate and submit XML sitemap to Google Search Console and Bing Webmaster Tools',
    priority: 'medium',
    status: 'pending'
  });
  ...
  checklist.push({
    ...
    description: biz.faqs && biz.faqs.length > 0 ? 
  ...
  checklist.push({
    ...
    description: 'Create strategic internal links between service pages, location pages, and blog content',
    priority: 'medium',
    status: 'pending'
  });
  ...
  checklist.push({
    ...
    description: 'Acquire links from local news sites, chambers of commerce, and industry associations',
    priority: 'medium',
    status: 'pending'
  });
