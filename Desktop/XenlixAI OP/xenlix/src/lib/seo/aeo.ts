...
export function buildAeoChecklist(biz: NormalizedBusiness): AeoChecklistItem[] {
  ...
  checklist.push({
    ...
    description: 'Upload 10+ recent photos including exterior, interior, products/services, and team photos',
    priority: 'high',
    status: 'pending'
  });
  ...
  checklist.push({
    ...
    description: 'Set up and optimize your Bing Places for Business listing with complete NAP information',
    priority: 'medium',
    status: 'pending'
  });
  ...
  checklist.push({
    ...
    priority: 'high',
    status: hasReviews && biz.rating! >= 4.0 && biz.reviewCount! >= 25 ? 'complete' : 'needs-attention'
  });

  checklist.push({
    category: 'Reviews',
    title: 'Respond to All Reviews',
    description: 'Implement process to respond to all reviews within 24 hours, especially negative ones',
    priority: 'high',
    status: 'pending'
  });
  ...
  checklist.push({
    ...
    description: 'Verify identical Name, Address, Phone across all online directories and platforms',
    priority: 'high',
  ...
  checklist.push({
    ...
    description: 'Use tools like Moz Local or BrightLocal to monitor and fix citation inconsistencies',
    priority: 'medium',
    status: 'pending'
  });
  ...
  if (biz.faqs && biz.faqs.length > 0) {
    ...
    checklist.push({
      ...
      description: 'Develop 10+ frequently asked questions with detailed answers for AI training data',
      priority: 'medium',
