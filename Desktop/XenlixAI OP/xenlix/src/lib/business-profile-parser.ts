...
const FacebookBusinessSchema = z.object({
  ...
  category_list: z.array(z.object({
    id: z.string(),
    name: z.string(),
  })).optional(),
  location: z.object({
...
export class BusinessProfileParser {
  ...
  private determineFormatAndParse(data: unknown, source: string): NormalizedBusinessProfile {
    ...
    if ('overall_star_rating' in dataObj && 'rating_count' in dataObj && 'category_list' in dataObj) {
  ...
  private parseGoogleMyBusiness(data: Record<string, unknown>, source: string): NormalizedBusinessProfile {
    ...
    const avgRating = totalReviews > 0 
      ? reviews.reduce((sum: number, review: any) => sum + parseInt(review.starRating), 0) / totalReviews
      : 0;
    ...
    return {
      ...
      seo: {
        primaryKeywords: this.generateKeywords(gmb.name, gmb.categories?.[0], gmb.location?.address?.locality),
  ...
  private parseYelpBusiness(data: Record<string, unknown>, source: string): NormalizedBusinessProfile {
    ...
    return {
      ...
      seo: {
        primaryKeywords: this.generateKeywords(yelp.name, yelp.categories?.[0]?.title, yelp.location?.city),
  ...
  private parseFacebookBusiness(data: Record<string, unknown>, source: string): NormalizedBusinessProfile {
    ...
    return {
      ...
      location: {
        ...
        coordinates: (fb.location?.latitude && fb.location?.longitude) ? {
  ...
  private parseGenericBusiness(data: Record<string, unknown>, source: string): NormalizedBusinessProfile {
    ...
    if (business.reviews) {
      if (Array.isArray(business.reviews)) {
        ...
        reviewData.rating = reviews.length > 0 
   ...
   */
  public toCustomerProfile(normalized: NormalizedBusinessProfile): import('@/types/ads').CustomerProfile {
    return {
  ...
  public generateOptimizationRecommendations(profile: NormalizedBusinessProfile): {
    ...
    if (profile.reviews.count < 20) {
      recommendations.reviews.push('Implement review collection strategy to increase review volume');
    }
...
export function parseBusinessProfileFromJSON(jsonData: unknown, source?: string): NormalizedBusinessProfile {
...
export function validateBusinessProfile(profile: unknown): profile is NormalizedBusinessProfile {
  try {
    ...
    return !!(p.businessName && p.industry && p.city && p.reviews && p.location && p.seo && p.metadata);
  } catch {
    return false;
  }
}
...
export function mergeBusinessProfiles(
  ...
  return {
    ...
    // Combine reviews
    reviews: {
      rating: primary.reviews.count > secondary.reviews.count 
    ...
    // Merge SEO data
    seo: {
      primaryKeywords: [...new Set([...primary.seo.primaryKeywords, ...secondary.seo.primaryKeywords])],
      secondaryKeywords: [...new Set([...primary.seo.secondaryKeywords, ...secondary.seo.secondaryKeywords])],
      targetAudience: [...new Set([...primary.seo.targetAudience, ...secondary.seo.targetAudience])],
      competitiveAdvantages: [...new Set([...primary.seo.competitiveAdvantages, ...secondary.seo.competitiveAdvantages])],
      uniqueSellingPropositions: [...new Set([...primary.seo.uniqueSellingPropositions, ...secondary.seo.uniqueSellingPropositions])],
    },
