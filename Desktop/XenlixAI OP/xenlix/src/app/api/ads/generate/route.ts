...
function generateAdDrafts(request: GenerateAdsRequest): AdDraftBundle {
  ...
  const hasStrongReviews = Boolean(profile.reviews && profile.reviews.rating >= 4.5 && profile.reviews.count >= 50);
  ...
  return {
    google: generateGoogleDraft(profile, objective, usp, promos, baseKeywords, hasStrongReviews, isLowBudget),
...
function generateGoogleDraft(profile: CustomerProfile, objective: CampaignGoal, usp: string[], promos: string[], keywords: string[], hasStrongReviews: boolean, isLowBudget: boolean) {
  const businessName = profile.businessName || "Your Business";
  const industry = profile.industry || "Business";
  const city = profile.city || "Local";
...
function generateBingDraft(profile: CustomerProfile, objective: CampaignGoal, usp: string[], promos: string[], keywords: string[], hasStrongReviews: boolean) {
  const businessName = profile.businessName || "Your Business";
  const industry = profile.industry || "Business";
  const city = profile.city || "Local";
  const services = profile.services?.[0] || "Services";
...
function generateMetaDraft(profile: CustomerProfile, objective: CampaignGoal, usp: string[], promos: string[], hasStrongReviews: boolean) {
  const businessName = profile.businessName || "Your Business";
  const industry = profile.industry || "Business";
  const city = profile.city || "Local";
  const services = profile.services?.[0] || "Services";
  ...
  const callToAction = objective === 'leads' ? 'CONTACT_US' as const : 
                       objective === 'sales' ? 'SHOP_NOW' as const : 
                       'LEARN_MORE' as const;
...
function generateTikTokDraft(profile: CustomerProfile, objective: CampaignGoal, usp: string[], promos: string[]) {
  const businessName = profile.businessName || "Your Business";
